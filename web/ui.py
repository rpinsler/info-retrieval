...
def search():
    ...
    docs, metadata = searcher.search(query=query, adv_query=adv_query, N=config['topN'])
    snippet = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse laoreet mauris eu tortor" + \
    ...
    for (rank, doc) in enumerate(docs):
        ...
        results.append(dict(rank=rank+1, score=round(doc.score, 2),
                            key=d['id'], title=d['title'][:-1], text=snippet,
                            authors=authors,
