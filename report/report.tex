...
The document index can now be used to retrieve the most relevant documents to a given query, which may be a combination of keywords and phrases. Those search capabilities are bundled in the Searcher class presented in Listing \ref{lst:searcher}. The main search logic is contained within the \emph{search} function as outlined in Listing \ref{lst:search}, whereas phrase queries are processed using \emph{extract\_pq}.  
